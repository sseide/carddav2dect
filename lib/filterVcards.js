...
/** function to filter list of vCards based upon a distribution list vCard. The distribution list vCard must be part of
 *  the list with all vCards to filter upon.
 *  First the distribution list vCard is extracted and then all contacts referenced as members (xAddressbookserverMember)
...
async function readVCardFromFile(file) {
    ...
    if (file.endsWith('.vcf')) {
        try {
            ...
            debug('got vCard data from file - %s: %s', file,
              card.get(VCARD_FULLNAME)?.valueOf());
            return card;
        }
        catch (errRead) {
