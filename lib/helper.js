 ...
 *  @return absolute file name, no normaization for '../' is done if file does not exist
 */
...
module.exports.logInfo = function logInfo(msg) {
    const dt = new Date();
    console.info(`${dt.getFullYear()}-${dt.getMonth()}-${dt.getDay()} ${dt.getHours()}:${dt.getMinutes()}:${dt.getSeconds()} - ${msg}` )
};

module.exports.logErr = function logErr(err) {
    const dt = new Date();
    console.error(`${dt.getFullYear()}-${dt.getMonth()}-${dt.getDay()} ${dt.getHours()}:${dt.getMinutes()}:${dt.getSeconds()} - ERROR ${err.msg}` );
    console.error(err)
};
