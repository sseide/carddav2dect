...
async function uploadPhonebook(page) {
    ...
    const fileToUpload = path.join(__dirname, '..', config.get('destination.vCardFile'));
...
module.exports.upload = async function upload() {
    ...
    try {
        ...
        const confHandsets = config.get('destination.gigaset.handsets');
        // loop over all handsets and select each one that is configure in config, filter invalid names
