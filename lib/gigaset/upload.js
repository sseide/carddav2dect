...
/**
 * older Puppeteer had "page.waitFor(timeout)", later renamed to "page.waitForTimeout(timeout)" which
 * is deprecated now. Can be replaced with this code (as taken from docs).
 * As Gigaset pages are slow in loading or doing things it is sometimes needed...
 *
 * @param milliseconds time to wait in milliseconds
 * @returns {Promise<unknown>} Promise that waits given time
 */
...
/** event handler for upload dialog - the dialog is shown at the end either saying everything
 *  was sent or an error occurred, Just log message to console and close dialog.
 *
 * @param dialog ref to js dialog window
 * @returns {Promise<void>}
 */
...
async function uploadPhonebook(page) {
    helper.logInfo('Upload new phone book file');
...
module.exports.upload = async function upload() {
    ...
    if (config.has('chrome.headless')) launchParam.headless = config.get('chrome.headless');

    // following hack to run chromium inside docker container - disable sandbox here as we cannot
    // enable kernel namespace cloning within the container without giving it capability SYS_ADMIN (root equivalent)
    // disabling sandbox for accessing Gigaset webui is less risky inside throw-away container than running as privileged
    // root user
    if (config.get('chrome.disableSandbox')) launchParam.args = ['--no-sandbox', '--disable-setuid-sandbox'];
    ...
    try {
        ...
        for (const handset of useHandsets) {
